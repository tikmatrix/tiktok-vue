  ...
  <div class="w-full">
    <Pagination :items="avatars" :searchKeys="['name']" @refresh="get_avatars">
      <template v-slot:buttons>
        <MyButton @click="add_avatar" label="add" icon="fa fa-add" />
      </template>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(avatar, index) in slotProps.items" :key="index">
                ...
                <td>
                  <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" @click="delete_avatar(avatar)">{{ $t('delete') }}</button>
                </td>
              </tr>
            </tbody>
          </table>
