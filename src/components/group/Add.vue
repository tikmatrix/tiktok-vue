<template>
  <div class="bg-base-100 flex flex-col items-start p-4">
    ...
    <div class="grid grid-cols-8 w-full items-center gap-2 mb-2">
      ...
      <input type="checkbox" class="toggle toggle-accent col-span-1" v-model="mygroup.auto_train" true-value="1" false-value="0" />
  ...
  methods: {
    add() {
      ...
      this.mygroup.publish_start_time = [this.publish_time1, this.publish_time2, this.publish_time3, this.publish_time4, this.publish_time5, this.publish_time6]
        .filter(Boolean)
        .join(',')
      if (!this.mygroup.train_start_time.match(/^(\d{2}:\d{2},)*\d{2}:\d{2}$/)) {
        alert('train_start_time format error')
        return
      }
      if (!this.mygroup.publish_start_time.match(/^(\d{2}:\d{2},)*\d{2}:\d{2}$/)) {
        alert('publish_start_time format error')
        return
      }
      if (this.mygroup.id) {
        this.$emit('update', this.mygroup)
      } else {
        this.$emit('add', this.mygroup)
      }
    }
  },
  mounted() {
