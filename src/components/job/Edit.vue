<template>
    <div class="flex flex-col items-start p-4">
        <div class="grid grid-cols-2 items-center gap-2 mb-2 w-4/5">
            <label class="font-bold text-right">{{ $t('startTime') }}</label>
            <VueDatePicker class="border-2 border-gray-300 p-2 rounded w-full" v-model="job.start_time"
                format="yyyy-MM-dd HH:mm:ss" position="center" locale="zh" time-picker-inline></VueDatePicker>
        </div>
        <div class="grid grid-cols-2 items-center gap-2 mb-2 w-4/5">
            <label class="font-bold text-right">{{ $t('material') }}</label>
            <input class="border-2 border-gray-300 p-2 rounded w-full" v-model="job.material" @click="openMaterialLibrary"
                readonly />
            <SelectMaterials class="border-2 border-gray-300 p-2 rounded col-span-3" v-if="showMaterialLibrary"
                @select="selectMaterial" />
        </div>
        <div class="grid grid-cols-2 items-center gap-2 mb-2 w-4/5">
            <label class="font-bold text-right">{{ $t('account') }}</label>
            <div class="relative">
        ...
        <div class="grid grid-cols-2 items-center gap-2 mb-2 w-4/5">
            <label class="font-bold text-right">{{ $t('title') }}</label>
            <input class="border-2 border-gray-300 p-2 rounded w-full" v-model="job.title" />
        </div>
        <div class="grid grid-cols-2 items-center gap-2 mb-2 w-4/5">
            <label class="font-bold text-right">{{ $t('tags') }}</label>
            <div class="border-2 border-gray-300 p-2 rounded w-full flex flex-wrap">
        ...
        <div class="grid grid-cols-2 items-center gap-2 mb-2 w-4/5">
            <label class="font-bold text-right">{{ $t('status') }}</label>
            <select class="border-2 border-gray-300 p-2 rounded w-full" v-model="job.status">
